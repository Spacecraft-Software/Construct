...
description: Use for writing provably-correct safety-critical Ada and SPARK code following Spacecraft Software standards. Triggers on any request involving Ada, SPARK, gnatprove, formal verification, design-by-contract (Pre/Post/Contract_Cases/Type_Invariant), proof of absence of run-time errors, SPARK_Mode, strong typing with constrained subtypes, ownership/access types, ghost code, Loop_Variant and loop invariants, Global/Depends aspects, or Ravenscar/Jorvik real-time tasking. Also triggers on .ads/.adb/.gpr files, Alire/alr, GNAT, gnatpp, gnatcheck, or any high-assurance component (bootloader, kernel path, crypto primitive, control loop) needing machine-checked correctness beyond what Rust guarantees. Do NOT trigger for plain Rust, C, or C++ work (use `spacecraft-rust-guidelines`, `spacecraft-clang-guidelines`, or `spacecraft-cpp-guidelines`) unless the task explicitly calls for that proof. By Mohamed Hammad and Spacecraft Software.
