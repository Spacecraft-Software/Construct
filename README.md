...
| [`spacecraft-flutterdart-guidelines`](spacecraft-flutterdart-guidelines/) | Type-safe highly-concurrent Dart & Flutter guidance — Sound Null Safety (avoiding `!`), multithreaded isolate task loops (`Isolate.run`), event-loop concurrency, Flutter rendering checks (`const` caching and `RepaintBoundary`), widget testing, and explicit controller disposal. |
...
| [`steelbore-color-palette`](steelbore-color-palette/) | Single source of truth for the Steelbore palette family (Standard §11 v1.35) — six palettes (Modern the default, Classic, Blue, BlackPinkPanther, MatrixGreen, NavyWhite), 13 themes with per-background WCAG matrices, the §11.1 role-token contract, the §11.1.1 accessibility variants, and the shipped `assets/steelbore.toml` + `assets/spacecraft.css`. Consumer skills defer here for color values. |
