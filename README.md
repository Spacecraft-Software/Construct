...
| [`steelbore-color-palette`](steelbore-color-palette/) | Single source of truth for the Steelbore 2 color palette (Standard §11 v1.34) — canonical hex tokens, the measured WCAG contrast matrix, the §11.1 `Steelbore` theme contract, the §11.1.1 accessibility variants, and the shipped `assets/steelbore.toml` + `assets/spacecraft.css`. Consumer skills defer here for color values. |
