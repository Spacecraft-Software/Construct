...
**Scope.** Concrete Rust scaffolds for every piece of the Steelbore CLI
...
(memory-safe, preferably Rust, Steelbore-compatible).
...
| Purpose | Crate |
|---------|-------|
| MCP server SDK | Current Steelbore-recommended MCP Rust SDK (consult `rust-guidelines` references for the specific crate name at time of writing). Alternative: hand-roll over `jsonrpc-core` for stdio transport. |
...
Steelbore-recommended MCP Rust SDK for the transport layer. The key is
