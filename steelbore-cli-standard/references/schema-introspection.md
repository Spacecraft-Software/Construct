...
**Scope.** How a Steelbore CLI describes itself at runtime so agents never
...
Every Steelbore CLI MUST implement:
...
```json
{
  ...
  "parameters": {
    ...
    "properties": {
      ...
      "algorithm": {
        "type": "string",
        "enum": ["potrace", "autotrace", "steelbore-native"],
        "default": "steelbore-native",
...
All Steelbore CLIs use a consistent hierarchy:
...
These verbs MUST be used consistently across all Steelbore tools. If a
...
Every Steelbore CLI repository MUST include these four files at the repo
...
| `SKILL.md` | Steelbore skill file with YAML frontmatter: command capabilities, constraints, usage patterns | Any skill-aware AI runtime (Claude Code skills, CLI-Anything, `gws`, custom harnesses) |
...
description: >
  ...
  should invoke it. Follow the Steelbore skill description conventions.
license: GPL-3.0-or-later
---
```
