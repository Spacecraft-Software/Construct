...
keybindings, the Steelbore palette mapping, and implementation constraints
(ratatui + crossterm).
...
The TUI MUST use the Steelbore six-token palette exclusively. No ad-hoc
...
(Steelbore's Share Tech Mono / Inconsolata preferences apply to the user's
...
- **Recommended stack:** `ratatui` for widgets + `crossterm` for the terminal backend. Both are pure Rust and Steelbore-compatible (see `steelbore-cli-preference` / references for the broader ecosystem).
...
- Using hard-coded colors instead of palette tokens. Violates the Steelbore brand.
