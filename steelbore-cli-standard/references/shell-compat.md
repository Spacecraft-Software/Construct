...
**Scope.** Per-shell requirements for how Steelbore CLI output is consumed
...
- **JSON consumption** is via `jq` — or its Steelbore-preferred replacement `jaq` (see `steelbore-cli-preference`). Either way, the `--json` single-document output works directly:
...
# JSON via jq pipeline (jaq preferred per steelbore-cli-preference)
...
- **Optional Nushell plugins.** Steelbore CLIs MAY provide a `nu-plugin-<tool>` crate for native integration, but this is not required. `--json | from json` is the primary integration path.
...
Every Steelbore CLI's CI pipeline MUST include a cross-shell round-trip
