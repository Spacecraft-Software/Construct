...
**Scope.** Everything about what a Steelbore CLI emits to stdout and stderr:
...
CLICOLOR), the Steelbore palette tokens for terminal output, and the exact
...
- **Color palette.** Use the Steelbore six-token palette via ANSI escape sequences, with semantic mapping:
