...
Reference for producing PDF deliverables under the Steelbore document-format skill. Load this **only when a PDF is requested**, in addition to whichever authoring reference produced the source file.

PDF is **not an authored format** in Steelbore. Always render PDF from an existing tier-1 file (preferably ODF; alternatively MS Office) via LibreOffice headless.
...
For Steelbore archival deliverables, **PDF/A-2** is the default. Use PDF/A-3 only when you want to embed the source file alongside the PDF (useful for fully-self-contained legal submissions; otherwise unnecessary overhead).
...
The `emb yes` column on both Share Tech Mono and Inconsolata is the pass condition. If either reads `no`, the PDF will render with a fallback font on systems missing the originals — visually wrong on Steelbore deliverables.
