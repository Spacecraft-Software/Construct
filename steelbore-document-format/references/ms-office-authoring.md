...
Reference for producing `.docx` (Word), `.xlsx` (Excel), and `.pptx` (PowerPoint) deliverables under the Steelbore document-format skill. Load this **only when an MS Office format is explicitly requested**, alongside `markdown-companion.md`.
...
**Without the `<w:displayBackgroundShape/>` flag, Word hides the background in Print Layout view on ~80% of installs.** This is the most common Steelbore-DOCX gotcha — the file *opens* showing Void Navy in Web Layout but flips to white in Print Layout, and most users default to Print Layout. Both flags or neither.

Verify by saving the file, closing Word, reopening in Print Layout — Void Navy must be visible.

### §B.2 — `.xlsx` (Excel)

XLSX has no native "sheet background colour" property the way HTML does. Two paths exist; use **path 1** for Steelbore work:
...
- Print/export to PDF.
- CSV round-trip (the colour is lost in CSV but the structural data is preserved; on re-import to Excel, you re-apply the Steelbore template).
...
Also set the sheet tab colour to a Steelbore accent (`#4B7EB0` Steel Blue is the default choice) via `<sheetPr><tabColor rgb="FF4B7EB0"/></sheetPr>` in each sheet's XML — cosmetic but matches the brand.
...
XLSX does not natively support font embedding the way DOCX does — readers fall back to system fonts. Both Steelbore fonts are on Google Fonts and ship with most Linux distros; readers without them get the system monospace fallback (acceptable).
...
2. **`soffice --headless --convert-to docx <input.odt>`** — converts a known-good ODT (built per `odf-authoring.md`) into DOCX. Round-trips Steelbore styles cleanly because LibreOffice's DOCX writer is mature. Recommended when an ODT version already exists.
...
2. **`soffice --headless --convert-to xlsx <input.ods>`** — converts a Steelbore ODS into XLSX. Cell fills survive cleanly.
...
2. **`soffice --headless --convert-to pptx <input.odp>`** — converts a Steelbore ODP into PPTX. Master + layout backgrounds round-trip.

## §F — Pitfalls and gotchas

- **Forgotten `w:displayBackgroundShape` in DOCX** — the #1 Steelbore-DOCX bug. Always verify post-generation.
...
- **Font availability on the reader's machine** — XLSX doesn't embed fonts; Inconsolata and Share Tech Mono fall back to system monospace if missing. Acceptable because both fonts are pre-installed on Steelbore hosts and Google Fonts handles the web/Google Workspace case.
