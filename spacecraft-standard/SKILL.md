...
**External-compliance posture (GNU/FSF).** A direct contradiction is not a strictness gradient: an artifact whose goal is to be **GNU-compliant** — an official GNU package, upstreamed to GNU/FSF, or hosted on Savannah — must meet GNU/FSF requirements that flatly oppose this Standard's *identity* clauses (§2 naming, §11–§12 branding, §15 attribution, and GitHub hosting). Such an artifact MAY declare the **pure-FSF posture** (the `pure-fsf-gnu` skill); in that posture GNU/FSF requirements override those identity clauses for that artifact, while this Standard's GNU-silent clauses — §6.3 signed commits, §14 UTC dates, §3.3 security-by-design — still apply and stack. Absent that declaration (the default for every Spacecraft project), this Standard governs in full and GNU conventions are adopted only where they aid interoperability (the GNU-*compatible* posture; see the `gnu-coding-standards` skill).
